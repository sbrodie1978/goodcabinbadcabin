...
<title>Good Cabin Bad Cabin — Princess Cruises cabin ratings</title>
...
.phead{display:flex;flex-wrap:wrap;gap:10px 18px;align-items:center;border-bottom:1px solid var(--line);padding:16px 20px}
.pmeta{display:flex;flex-wrap:wrap;gap:8px;align-items:center}
...
/* hero overall score — centred */
.hero{display:flex;flex-direction:column;align-items:center;text-align:center;gap:5px;border:2px solid var(--line);padding:16px;margin-bottom:18px}
.hero .heronum{font-family:var(--disp);font-weight:400;font-size:66px;line-height:.82;letter-spacing:.01em}
.hero .heroden{font-family:var(--mono);font-size:17px;font-weight:500;opacity:.55;margin-left:1px}
.hero .herotxt{display:flex;flex-direction:column;gap:3px}
.hero .heroverdict{font-family:var(--mono);font-size:15px;font-weight:600;text-transform:uppercase;letter-spacing:.07em}
.hero .heroprof{font-family:var(--mono);font-size:10.5px;color:var(--ink2);text-transform:uppercase;letter-spacing:.1em}
/* per-dimension: big number, slim bar */
.srow{margin-bottom:11px}
.srow .t{display:flex;justify-content:space-between;align-items:baseline;font-family:var(--mono)}
.srow .t .sl{font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:var(--ink2)}
.srow .t .sv{font-size:21px;font-weight:600;line-height:1}
.bar{height:4px;background:var(--paper);border:1px solid var(--line);margin-top:4px}
.bar i{display:block;height:100%}
.scorenote{font-family:var(--mono);font-size:10px;color:var(--ink2);letter-spacing:.02em;margin-top:2px}
/* ranks: prominent numbers, centred */
.ranks{margin-top:16px;padding-top:14px;border-top:1px dashed var(--line);text-align:center}
.rankrow{display:flex;align-items:baseline;justify-content:center;gap:9px;margin-bottom:5px}
.rankrow .rn{font-family:var(--disp);font-weight:400;font-size:30px;line-height:.9;color:var(--ink)}
.rankrow .rl{font-family:var(--mono);font-size:12px;color:var(--ink2)}
.ranks .rprof{font-family:var(--mono);font-size:10px;color:var(--ink2);text-transform:uppercase;letter-spacing:.08em;margin-top:6px}
...
.copylink{font-family:var(--mono);font-size:10.5px;letter-spacing:.08em;text-transform:uppercase;margin-left:auto;
  border:1.5px solid var(--line);background:#fff;color:var(--ink2);padding:5px 9px;cursor:pointer;border-radius:0}
.copylink:hover{border-color:var(--ink);color:var(--ink)}
.copylink.ok{border-color:var(--good);color:var(--good)}
.copylink:focus-visible{outline:2px solid var(--accent);outline-offset:1px}
...
/* force an always-visible horizontal scrollbar so desktop mice can pan too
   (macOS hides overlay scrollbars, which otherwise strands the aft cabins) */
.stripwrap,.planwrap{scrollbar-width:thin;scrollbar-color:var(--ink2) var(--card)}
.stripwrap::-webkit-scrollbar,.planwrap::-webkit-scrollbar{height:10px}
.stripwrap::-webkit-scrollbar-track,.planwrap::-webkit-scrollbar-track{background:var(--card)}
.stripwrap::-webkit-scrollbar-thumb,.planwrap::-webkit-scrollbar-thumb{background:var(--ink2);border:2px solid var(--card)}
...
@media(max-width:640px){
  ...
  .pnum{font-size:44px}
  .hero .heronum{font-size:60px}
  .phead{flex-direction:column;align-items:center;text-align:center;padding:16px 14px;gap:11px}
  .pmeta{justify-content:center}
  .phead .copylink{margin-left:0}
...
/* view toggle + deck selector + deck-plan */
.viewbar{display:flex;flex-wrap:wrap;gap:12px 16px;align-items:center;margin:20px 0 4px}
.viewtoggle button{font-size:11.5px;padding:9px 14px}
.decksel{display:flex;flex-wrap:wrap;gap:4px;align-items:center}
.decksel .dlab{font-family:var(--mono);font-size:10px;letter-spacing:.16em;text-transform:uppercase;color:var(--ink2);margin-right:2px}
.decksel button{font-family:var(--mono);font-size:12px;padding:6px 10px;background:#fff;border:1.5px solid var(--line);cursor:pointer;color:var(--ink2)}
.decksel button:hover{border-color:var(--ink);color:var(--ink)}
.decksel button[aria-pressed="true"]{background:var(--ink);color:#fff;border-color:var(--ink)}
.decksel button:focus-visible{outline:2px solid var(--accent);outline-offset:1px}
.planwrap{border:1px solid var(--line);background:var(--card);overflow-x:auto;-webkit-overflow-scrolling:touch}
#deckplan{display:block;min-width:100%;background:var(--card)}
#deckplan .pcab{cursor:pointer;stroke:rgba(0,0,0,.28);stroke-width:.6;vector-effect:non-scaling-stroke}
#deckplan .pcab:hover{stroke:var(--ink);stroke-width:1.6}
#deckplan .pcab.sel{stroke:var(--ink);stroke-width:2.4}
#deckplan .pcab.dim{opacity:.28}
#deckplan .hull{fill:none;stroke:var(--line);stroke-width:1}
#deckplan .sidelab{font-family:var(--mono);font-size:11px;letter-spacing:.12em;fill:var(--ink2);text-transform:uppercase}
.planempty{padding:26px 20px;font-family:var(--mono);font-size:12px;color:var(--ink2)}
@media(max-width:640px){
  #deckplan{min-width:820px}
  .viewtoggle{display:flex;grid-template-columns:none}
...
<header><div class="wrap">
  <div style="margin-bottom:7px"><a href="../" style="font-family:var(--mono);font-size:11px;letter-spacing:.1em;text-decoration:none">← ALL CRUISE LINES</a></div>
...
<div class="wrap" id="pickerview">
  <div class="striplabel" style="margin-top:22px"><span>PRINCESS CRUISES</span><span></span><span>CHOOSE YOUR SHIP</span></div>
  <div id="lines"></div>
</div>
...
<div class="wrap" id="appview" style="display:none">
  <div class="controls">
    ...
    <div class="field"><label for="cat">Cabin type</label>
      <select id="cat"><option value="">All cabin types</option></select></div>
  </div>

  <div class="viewbar">
    <div class="seg viewtoggle" role="group" aria-label="Map view" id="viewtoggle">
      <button data-v="plan" aria-pressed="true">Deck plan</button>
      <button data-v="strip" aria-pressed="false">Whole-ship overview</button>
    </div>
    <div class="decksel" id="decksel" aria-label="Choose a deck"></div>
  </div>

  <div class="striplabel"><span>◀ FORWARD</span><span id="maptitle">EVERY CABIN, COLOURED BY SCORE — CLICK ONE</span><span>AFT ▶</span></div>
  <div class="stripwrap" id="stripwrap" style="display:none"><svg id="ship" viewBox="0 0 1040 330" role="img" aria-label="Ship map of all cabins coloured by score"></svg></div>
  <div class="planwrap" id="planwrap"><svg id="deckplan" viewBox="0 0 1000 220" role="img" aria-label="Deck plan of cabins coloured by score"></svg></div>
  <div class="swipehint" aria-hidden="true">↔ scroll sideways to see the whole ship — bow to aft</div>
  <div class="legend">
...
<footer><div class="wrap">Independent tool made by a cruiser — not affiliated with or endorsed by Princess Cruises or any cruise line.<br>Part of <a href="https://thecruiselab.com">The Cruise Lab</a> · <a href="../">all cruise lines</a></div></footer>
...
// core cabin types — the app groups, filters and ranks by these five, not the
// granular sub-categories (Premium Oceanview, Deluxe Balcony, category codes, …)
const TYPE_NAME = {interior:"Interior",oceanview:"Oceanview",balcony:"Balcony","mini-suite":"Mini-suite",suite:"Suite"};
const TYPE_ORDER = ["interior","oceanview","balcony","mini-suite","suite"];
let profile = "overall", typeFilter = "", SHIP=null, DECKS=[], cabs=[], byNum={}, curNum=null;
let view = "plan", currentDeck = null;

// ---- URL state (shareable deep links: #shipId/cabin/profile) ----
let _suppressHash=false;
function setHash(h){ if(location.hash===h)return; _suppressHash=true; location.hash=h;
  setTimeout(()=>{_suppressHash=false;},0); }
function setProfileButtons(k){ profile=k;
  [...pWrap.children].forEach(x=>x.setAttribute("aria-pressed",x.dataset.p===k)); }
async function route(){
  const h=decodeURIComponent((location.hash||"").replace(/^#/,""));
  if(!h){picker();return;}
  const [idStr,num,prof]=h.split("/");
  const s=SHIPS.find(x=>String(x.id)===idStr);
  if(!s){picker();return;}
  if(!SHIP||SHIP.id!==s.id) await loadShip(s,true);
  if(prof&&PROFILES[prof]) setProfileButtons(prof);
  if(num&&byNum[num]){ show(num,true); }
  else { render(); document.getElementById("panel").classList.remove("open"); curNum=null; }
}
window.addEventListener("hashchange",()=>{ if(_suppressHash)return; route(); });
function copyLink(btn){
  const url=location.href, done=()=>{const t=btn.textContent;btn.textContent="✓ copied";
    btn.classList.add("ok");setTimeout(()=>{btn.textContent=t;btn.classList.remove("ok");},1400);};
  if(navigator.clipboard&&navigator.clipboard.writeText){
    navigator.clipboard.writeText(url).then(done,()=>prompt("Copy this link:",url));
  } else prompt("Copy this link:",url);
}

async function loadShip(s,fromRoute){
  SHIP=s; DECKS=s.decks; typeFilter=""; curNum=null; currentDeck=null;
  if(!fromRoute) setHash(String(s.id));
  if(!s.data){
    document.getElementById("shipname").textContent="Loading "+s.name+"…";
    try{ s.data = await fetch("data/ship-"+s.id+".json?v="+DV).then(r=>r.json()); }
  ...
  cabs=s.data.map(r=>({num:r[0],deck:r[1],side:r[2],b:r[3],q:r[4],st:r[5],cv:r[6],
    ...
    color:r[16]||"#cfd6cf",sur:r[17]||null,type:r[18]||null}));
  ...
  sel.innerHTML='<option value="">All cabin types</option>';
  const present=new Set(cabs.map(c=>c.type).filter(Boolean));
  const types=TYPE_ORDER.filter(t=>present.has(t));
  types.forEach(t=>{const o=document.createElement("option");o.value=t;o.textContent=TYPE_NAME[t];sel.appendChild(o);});
  sel.parentElement.style.display=types.length>1?"block":"none";
  const H=30, vb=8+DECKS.length*(H+2)+4;
  document.getElementById("ship").setAttribute("viewBox","0 0 1040 "+vb);
  render();
}
function picker(){
  setHash(""); SHIP=null; curNum=null;
...
function ranks(p){
  ...
  const g={};
  cabs.forEach(c=>{(g[c.type]=g[c.type]||[]).push(c)});
...
for(const k in PROFILES){
  ...
  b.onclick=()=>{profile=k;[...pWrap.children].forEach(x=>x.setAttribute("aria-pressed",x.dataset.p===k));
    if(curNum&&byNum[curNum])show(curNum);else render();};
  pWrap.appendChild(b);
}
document.getElementById("cat").onchange=e=>{typeFilter=e.target.value;render();};

// ---- ship strip ----
const svg=document.getElementById("ship"), tip=document.getElementById("tip");
// only show the hover tooltip on devices that actually hover — on touch there is
// no mouseleave to hide it, so it would stick to the screen after a tap.
const CAN_HOVER = !(window.matchMedia && window.matchMedia("(hover: none)").matches);
// let a vertical mouse-wheel pan the map horizontally (desktop mice can't drag
// the scrollbar on macOS); releases to the page once an edge is reached.
function enableWheelPan(el){ if(!el)return;
  el.addEventListener("wheel",e=>{
    if(el.scrollWidth<=el.clientWidth) return;
    if(Math.abs(e.deltaX)>Math.abs(e.deltaY)) return;   // real horizontal input (trackpad) — leave it
    const max=el.scrollWidth-el.clientWidth;
    if((e.deltaY<0&&el.scrollLeft>0)||(e.deltaY>0&&el.scrollLeft<max-1)){
      el.scrollLeft=Math.max(0,Math.min(max,el.scrollLeft+e.deltaY)); e.preventDefault();
    }
  },{passive:false});
}
enableWheelPan(document.getElementById("stripwrap"));
enableWheelPan(document.getElementById("planwrap"));
function updateHint(){
  const hint=document.querySelector(".swipehint"); if(!hint)return;
  const el=view==="strip"?document.getElementById("stripwrap"):document.getElementById("planwrap");
  hint.style.display=(el&&el.scrollWidth>el.clientWidth+2)?"block":"none";
}
...
function drawShip(){
  ...
  DECKS.forEach((d,i)=>{
    ...
    row.forEach(c=>{
      ...
      const dim=typeFilter&&c.type!==typeFilter;
      ...
      r.addEventListener("mousemove",e=>{ if(dim||!CAN_HOVER)return;
        tip.style.display="block";tip.style.left=(e.clientX+12)+"px";tip.style.top=(e.clientY-10)+"px";
        tip.textContent=`${c.num} · ${c.type?TYPE_NAME[c.type]:"?"} · ${score(c,profile)}`;});
...
function surroundSVG(c){
  ...
  const resolve=(cellData,fallbackLabel)=>{
    ...
    if(kind==="cabin"){
      ...
      if(nb){ const sc=score(nb,profile);
        return {label, sub:(nb.type?TYPE_NAME[nb.type]:"")+" · scores "+sc, color:col(sc), kind:"cabin", sc}; }
  ...
  const meCell={label:c.num+(c.type?" · "+TYPE_NAME[c.type]:""),color:col(meSc),kind:"cabin"};
  return `
  <div class="xwrap">
    <div class="xvert">
...
function show(num,fromRoute){
  const c=byNum[num]; if(!c)return;
  curNum=num; tip.style.display="none";
  if(!fromRoute) setHash(SHIP.id+"/"+num+(profile!=="overall"?"/"+profile:""));
  ranks(profile);
  const p=document.getElementById("panel");
  const typeN=c.type?cabs.filter(x=>x.type===c.type).length:0;
  const sc=score(c,profile);
  const scTxt=sc>=72?"var(--good)":sc>=58?"#8a6d12":"var(--poor)"; // legible text tone for the score
  const noteTxt=c.notes.replace(/\(([-+]\d+)\)/g,"").replace(/;/g," · ");
  p.innerHTML=`
   <div class="phead">
     <div class="pnum">${c.num}</div>
     <div class="pmeta">
       <span class="chip">${c.type?TYPE_NAME[c.type]:"Type unknown"}</span>
       ${c.sqft?`<span class="chip">${c.sqft} sq ft${c.berths?" · sleeps "+c.berths:""}</span>`:""}
       ${c.conf==="guess"?'<span class="chip warn">type unverified</span>':""}
     </div>
     <div class="ploc">DECK ${+c.deck} · ${c.side==="P"?"PORT":"STARBOARD"} · ${c.b<0.33?"FORWARD":c.b<0.62?"MIDSHIP":"AFT"}</div>
     <button class="copylink" onclick="copyLink(this)" title="Copy a link straight to this cabin">🔗 copy link</button>
   </div>
   <div class="pbody">
    <div class="scores">
      <div class="hero">
        <div class="heronum" style="color:${scTxt}">${sc}<span class="heroden">/100</span></div>
        <div class="herotxt">
          <div class="heroverdict" style="color:${scTxt}">${verdictWord(sc)}</div>
          <div class="heroprof">${PROFILES[profile].label} profile</div>
        </div>
      </div>
      ${[["Quietness",c.q],["Motion",c.st],["Convenience",c.cv],["Space & comfort",c.space]].map(([t,v])=>`
        <div class="srow"><div class="t"><span class="sl">${t}</span><b class="sv" style="color:${col(v)}">${v}</b></div>
        <div class="bar"><i style="width:${v}%;background:${col(v)}"></i></div></div>`).join("")}
      <div class="scorenote">Higher is better · Motion 100 = you'll barely feel the sea</div>
      <div class="ranks">
        ${c.type?`<div class="rankrow"><span class="rn">#${c._cr}</span><span class="rl">of ${typeN} ${TYPE_NAME[c.type].toLowerCase()} cabins</span></div>`:""}
        <div class="rankrow"><span class="rn">#${c._r}</span><span class="rl">of ${cabs.length} cabins shipwide</span></div>
        <div class="rprof">ranked for the ${PROFILES[profile].label.toLowerCase()} profile</div>
      </div>
    </div>
    <div class="stack">
  ...
  if(view==="plan") syncPlanToCabin(c);
}

// ---- deck plan view (real cabin footprints, one deck at a time) ----
const dplan=document.getElementById("deckplan");
async function ensurePlan(){
  if(SHIP.plan) return true;
  try{ SHIP.plan=await fetch("data/ship-"+SHIP.id+"-plan.json?v="+DV).then(r=>{if(!r.ok)throw 0;return r.json();}); }
  catch(e){ SHIP.plan=null; return false; }
  return true;
}
function planDecks(){ return DECKS.filter(d=>SHIP.plan&&SHIP.plan.decks[String(+d)]); }
function defaultDeck(){
  if(curNum&&byNum[curNum]){const d=String(+byNum[curNum].deck); if(SHIP.plan.decks[d])return d;}
  let best=null,bn=-1;
  for(const d of planDecks()){const n=Object.keys(SHIP.plan.decks[String(+d)].cab).length; if(n>bn){bn=n;best=String(+d);}}
  return best;
}
function buildDeckSel(){
  const host=document.getElementById("decksel"); host.innerHTML="";
  const lab=document.createElement("span");lab.className="dlab";lab.textContent="Deck";host.appendChild(lab);
  planDecks().forEach(d=>{
    const dd=String(+d), b=document.createElement("button");
    b.textContent=(+d); b.dataset.d=dd; b.setAttribute("aria-pressed",dd===currentDeck);
    b.onclick=()=>{currentDeck=dd; buildDeckSel(); drawPlan(dd);
      document.getElementById("maptitle").textContent="DECK "+(+dd)+" — TAP ANY CABIN";};
    host.appendChild(b);
  });
}
function drawPlan(deck){
  const D=SHIP.plan&&SHIP.plan.decks[String(+deck)];
  dplan.innerHTML="";
  if(!D){ dplan.setAttribute("viewBox","0 0 1000 120");
    const t=document.createElementNS(dplan.namespaceURI,"text");
    t.setAttribute("x",20);t.setAttribute("y",60);t.setAttribute("class","sidelab");
    t.textContent="No deck-plan geometry for this deck."; dplan.appendChild(t); return; }
  const PADY=18, W=D.w, H=D.h+PADY*2;
  dplan.setAttribute("viewBox","0 0 "+W+" "+H);
  const mk=(x,y,txt)=>{const t=document.createElementNS(dplan.namespaceURI,"text");
    t.setAttribute("x",x);t.setAttribute("y",y);t.setAttribute("class","sidelab");t.textContent=txt;dplan.appendChild(t);};
  mk(6,13,"PORT ▲"); mk(6,H-5,"STARBOARD ▼");
  for(const num in D.cab){
    const c=byNum[num]; if(!c) continue;
    const pts=D.cab[num]; let s="";
    for(let i=0;i<pts.length;i+=2){ s+=pts[i]+","+(pts[i+1]+PADY)+" "; }
    const sc=score(c,profile), dim=typeFilter&&c.type!==typeFilter;
    const poly=document.createElementNS(dplan.namespaceURI,"polygon");
    poly.setAttribute("points",s.trim());
    poly.setAttribute("fill",dim?"#d6dcd6":col(sc));
    poly.setAttribute("class","pcab"+(dim?" dim":"")+(num===curNum?" sel":""));
    poly.setAttribute("tabindex",dim?-1:0);
    poly.setAttribute("aria-label","Cabin "+num);
    if(!dim){
      poly.addEventListener("click",()=>show(num));
      poly.addEventListener("keydown",e=>{if(e.key==="Enter")show(num)});
      poly.addEventListener("mousemove",e=>{if(!CAN_HOVER)return;tip.style.display="block";
        tip.style.left=(e.clientX+12)+"px";tip.style.top=(e.clientY-10)+"px";
        tip.textContent=num+" · "+(c.type?TYPE_NAME[c.type]:"?")+" · "+sc;});
      poly.addEventListener("mouseleave",()=>tip.style.display="none");
    }
    dplan.appendChild(poly);
  }
  if(curNum){const sp=dplan.querySelector(".pcab.sel"); if(sp)dplan.appendChild(sp);} // selected on top
  updateHint();
}
async function drawPlanView(){
  if(!(await ensurePlan())){ view="strip"; render(); return; }
  if(!currentDeck||!SHIP.plan.decks[String(+currentDeck)]) currentDeck=defaultDeck();
  buildDeckSel();
  document.getElementById("maptitle").textContent="DECK "+(+currentDeck)+" — TAP ANY CABIN";
  drawPlan(currentDeck);
}
async function syncPlanToCabin(c){
  if(!(await ensurePlan())) return;
  const d=String(+c.deck); if(!SHIP.plan.decks[d]) return;
  currentDeck=d; buildDeckSel(); drawPlan(d);
  document.getElementById("maptitle").textContent="DECK "+(+d)+" — TAP ANY CABIN";
}
...
function render(){
  const strip=document.getElementById("stripwrap"), pw=document.getElementById("planwrap"), dsel=document.getElementById("decksel");
  [...document.querySelectorAll("#viewtoggle button")].forEach(b=>b.setAttribute("aria-pressed",b.dataset.v===view));
  if(view==="strip"){
    strip.style.display="block"; pw.style.display="none"; dsel.style.display="none";
    document.getElementById("maptitle").textContent="EVERY CABIN, COLOURED BY SCORE — CLICK ONE";
    drawShip();
  } else {
    strip.style.display="none"; pw.style.display="block"; dsel.style.display="flex";
    drawPlanView();
  }
}
...
if __name__ == "__main__":
    ...
    _HERE=os.path.dirname(os.path.abspath(__file__));_OUT=os.path.join(os.path.dirname(_HERE),"app","public","princess","index.html")
